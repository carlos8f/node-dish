describe('basic test', function () {
  ...
  it('serve gzipped', function (done) {
    ...
    request.get(baseUrl + '/').set('Accept-Encoding', 'gzip, deflate').end(function (res) {
      assert.equal(res.statusCode, 200);
  ...
  it('if-none-match', function (done) {
    request.get(baseUrl + '/').set('If-None-Match', eTag).end(function (res) {
  ...
  it('if-modified-since', function (done) {
    request.get(baseUrl + '/').set('If-Modified-Since', lastModified).end(function (res) {
      assert.equal(res.statusCode, 304);
      assert(!res.text);
      done();
    });
