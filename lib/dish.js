...
function Dish (data, options) {
  ...
  if (!this.findHeader('Content-Length')) {
    this.headers['Content-Length'] = Buffer.byteLength(str);
  }
...
Dish.prototype.serve = function (req, res, status) {
  ...
  this.gzip(function () {
    ...
    if (self.keepAlive && ((res.response && res.response.shouldKeepAlive) || res.shouldKeepAlive)) {
      headers['Keep-Alive'] = 'timeout=' + self.keepAlive;
      req.connection.setMaxListeners(0);
      req.connection.setTimeout(self.options.keepAlive, function () {
        req.connection.end();
      });
    }
